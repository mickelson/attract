	...
	std::vector<std::string> &image_names )
{
...
#ifndef NO_SWF
	for ( itr = non_image_names.begin();
			itr != non_image_names.end(); ++itr  )
	{
		if ( tail_compare( *itr, ".swf" ) )
		{
			movie_file = (*itr);
			break;
		}
	...
	if ( !movie_file.empty() )
	{
		...
		if (!m_swf->open_from_file( movie_file ))
		{
		...
		{
			std::cout << " - Loaded SWF: " << movie_file << " ("
