...
bool FeRomInfo::operator< ( FeRomInfo s )
{
...
std::string FeListInfo::get_current_layout_file()
{
...
std::string FeListInfo::state_as_output()
{
...
void FeListInfo::save( std::ofstream &f )
{
	using std::setw;
	using std::left;
	using std::endl;
...
bool FeRomList::apply_filter( const FeRomInfo &rom )
{
...
bool FeEmulatorInfo::get_artwork( const std::string &label, std::string &artwork )
{
...
void FeEmulatorInfo::dump( void )
{
	...
	for ( std::map<std::string,std::string>::iterator itl=m_artwork.begin();
...
void FeEmulatorInfo::save( const std::string &filename )
{
#ifdef FE_DEBUG
	...
	if ( outfile.is_open() )
	{
		...
		outfile << "# Generated by " << FE_NAME_AND_VERSION << endl 
					<< "#" << endl;
		...
		for ( std::map<string, string>::iterator itr=m_artwork.begin();
